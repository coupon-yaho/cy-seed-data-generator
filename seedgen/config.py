...
# 현재(미래) 회차 — PRD.md:296-300. (브랜드 index, 재고)
#
# 시드 실행 시각 +5분부터 **이어 붙여** 만든다. 앞의 셋이 곧 열리는 회차이고,
# 그 뒤로는 아래 UPCOMING_DAYS 만큼 브랜드를 돌려 가며 계속 채운다.
CURRENT_COUPONS: list[tuple[int, int]] = [(0, 10_000), (5, 3_000), (8, 2_000)]

# 앞으로 며칠치 회차를 미리 만들어 둘지.
#
# **0 이면 위 셋만 만든다.** 그 상태에서는 시드 실행 시각으로부터 반나절이면 마지막
# 회차가 닫히고, 그 뒤로는 열린 회차가 영영 없다 — 하루 뒤에 화면을 켜면 발급을
# 눌러 볼 대상이 하나도 없다. 시연·수동 테스트가 시드 시각에 묶이는 것이 그 이유다.
UPCOMING_DAYS: int = 30

# 하루에 몇 회차를 둘지.
#
# **꽉 채우지 않는다.** 한때 하루 네 회차를 이어 붙여 23시간을 덮었는데, 브랜드데이가
# 하루 종일 끊이지 않고 이어지는 것은 제품의 모양이 아니다. 비는 시간이 있는 편이
# 실제에 가깝고, 화면에서도 "다음 회차 대기" 상태를 볼 수 있다.
UPCOMING_PER_DAY: int = 2

# 미리 만들어 두는 회차의 재고. 위 셋(만 단위)보다 작게 잡는다 — 발급을 눌러 보는 것이
# 목적이지 부하를 재는 자리가 아니고, 재고가 크면 "품절" 을 못 보여 준다.
UPCOMING_STOCK: int = 200
